...
pub fn create_window<R: Runtime>(app: &tauri::AppHandle<R>, config: WindowConfig) -> tauri::Result<()> {
    ...
    println!("Creating window '{}' with title '{}'", config.window_id, config.title);
...
pub fn create_popup_window<R: Runtime>(
    ...
    println!("Creating popup window at mouse position: ({}, {})", mouse_x, mouse_y);
    ...
    let config = WindowConfig {
        ...
        min_width: None,
        min_height: None,
        position: WindowPosition::Coordinates { x: mouse_x as f64, y: mouse_y as f64 },
        resizable: false,
        maximizable: false,
        minimizable: false,
        closable: true,
...
fn create_chat_window<R: Runtime>(app: &tauri::AppHandle<R>, prefix: &str, context: &str) -> tauri::Result<()> {
    println!("{}", context);
...
pub fn create_direct_chat_window<R: Runtime>(app: &tauri::AppHandle<R>) -> tauri::Result<()> {
    create_chat_window(app, "direct", "No text selected - opening chat window directly")
}
...
pub fn create_fallback_chat_window<R: Runtime>(app: &tauri::AppHandle<R>) -> tauri::Result<()> {
    create_chat_window(app, "fallback", "Failed to read clipboard - opening chat window as fallback")
}
