...
async fn process_shortcut_trigger<R: Runtime>(app_handle: AppHandle<R>) {
    ...
    println!("Original clipboard content: '{}' (length: {})", 
    ...
    let unique_marker = format!("AI_TOOL_MARKER_{}", 
    ...
    if let Err(e) = app_handle.clipboard().write_text(&unique_marker) {
        println!("Failed to clear clipboard, continuing with original method: {}", e);
    } else {
    ...
    println!("New clipboard content: '{}' (length: {})", 
    ...
    let has_meaningful_content = !new_clipboard.trim().is_empty() && new_clipboard.trim().len() > 0;
    let is_not_marker = !new_clipboard.starts_with("AI_TOOL_MARKER_");
    
    // Strategy 1: Traditional change detection
    let text_was_selected_traditional = clipboard_changed && has_meaningful_content && is_not_marker;
    ...
    let has_substantial_content = new_clipboard.trim().len() >= substantial_text_threshold && is_not_marker;
    ...
    let text_was_selected = text_was_selected_traditional || 
                           has_substantial_content || 
                           is_duplicate_but_meaningful;
    
    println!("Detection results:");
    println!("  clipboard_changed={}, has_meaningful_content={}, is_not_marker={}", 
        clipboard_changed, has_meaningful_content, is_not_marker);
    println!("  traditional_detection={}, substantial_content={}, duplicate_but_meaningful={}", 
