...
#### Medium
- `src-tauri/src/ai_provider/gemini.rs:207` — manual `Box::pin` + recursive async pattern for retry. Verbose and hard to read. **Suggestion:** replace with a `loop` that increments `retry_count` and `sleep`s on retryable status.
- `src/components/ChatWindow.vue:390` and `:468` — `sendMessage` and `regenerateResponse` duplicate the bulk of their bodies. **Suggestion:** extract a `runAssistantTurn(history, userContent)` helper.
- `src-tauri/src/commands/window_commands.rs:9` vs `window_manager.rs:221` — chat windows created via `invoke('open_chat_window')` and via tray/shortcut use different builders with different capability sets. **Suggestion:** route all chat-window creation through one builder/enum.
- `src-tauri/src/data_manager/manager.rs:97` — `migrate_from_old_files` silently swallows read/parse failures. **Suggestion:** log a `warn!` on each `Err` branch.
- `src/components/OperationEditWindow.vue:221` — `operations` (Record) and `operationsArray` (Array) are two sources of truth kept in sync manually. **Suggestion:** keep only one as state and derive the other via `computed`.
- `src/utils/markdown.ts:79` — `addCustomClasses` post-processes HTML with a chain of regexes. **Suggestion:** parse with `DOMParser`, mutate the DOM, then serialize.
- `src-tauri/src/data_manager/types.rs:183` and `ChatHistoryWindow.vue:517` — default operation names hardcoded in Rust and Vue. **Suggestion:** have the backend expose operation metadata and let the frontend render from it.

#### Low
- `src/components/ChatWindow.vue:614` — `handleGlobalKeydown` uses a `switch` for a single case.
- `src/components/ChatWindow.vue:640` — `loadConversation`'s `map` has two branches that build nearly identical objects.
- `src/components/PopupWindow.vue:194` and `:312` — dynamic import of `getCurrentWindow` instead of static.
- `src/components/PopupWindow.vue:96` — `Emits` declares `close` and `operation-selected` but no parent listener exists.
- `src/components/SettingsWindow.vue:84` and `:167` — magic numbers `3000`/`1500`.
- `src/components/ChatHistoryWindow.vue:465` — `result` binding only used for `logger.debug`.
- `src/components/ChatHistoryWindow.vue:878` — badge background colors are raw hex inline.
- `src/components/ChatHistoryWindow.vue:487` — `formatTimestamp` uses `1000 * 60 * 60` magic numbers.
- `src/components/OnboardingWindow.vue:279` — `Emits` declares events only emitted in unreachable catch fallbacks.
- `src/components/OnboardingWindow.vue:364` — `validateApiKey` accepts any string containing `"AI"`.
- `src/components/AppConfirmDialog.vue` and `AppPromptDialog.vue` — share near-identical CSS.
- `src-tauri/src/lib.rs:33` — `get_configured_shortcut` five-level nested `if let`.
- `src-tauri/src/ai_provider/gemini.rs:569` — `supports_thinking_mode` is `#[allow(dead_code)]`.
- `src-tauri/src/ai_provider/gemini.rs:628` — `test_rate_limiter` does not verify rate limiting actually blocks.
- `src-tauri/src/data_manager/manager.rs:56` — `save_data` clones entire `AppData` on every save.
- `src-tauri/src/data_manager/types.rs:103` — `Operation.order: i32` admits negative values.
- `src-tauri/src/data_manager/commands.rs:111` — `load_conversation_messages` linear scan (acceptable at 100 entries).
- `src-tauri/src/data_manager/commands.rs:70` — conversation id can collide in same millisecond.
- `src/utils/markdown.ts:25` — `marked.parse(markdown) as string` hides `string | Promise<string>` union.
- `src/utils/formatters.ts:1` — `formatModelName` is Gemini-specific but undocumented.
